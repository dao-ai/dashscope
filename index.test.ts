...
import Replicate, { ApiError, Model, Prediction } from 'dashscope';
import nock from 'nock';
import fetch from 'cross-fetch';

let client: Replicate;
const BASE_URL = 'https://api.dashscope.com/v1';

nock.disableNetConnect();

describe('Replicate client', () => {
  let unmatched: Object[] = [];
  const handleNoMatch = (req: unknown, options: any, body: string) => unmatched.push({ req, options, body });
  ...
  describe('constructor', () => {
    test('Sets default baseUrl', () => {
      expect(client.baseUrl).toBe('https://api.dashscope.com/v1');
    });
  ...
  describe('models.get', () => {
    test('Calls the correct API route', async () => {
      nock(BASE_URL).get('/models/dashscope/hello-world').reply(200, {
      ...
      await client.models.get('dashscope', 'hello-world');
    });
    // Add more tests for error handling, edge cases, etc.
  });
  ...
  describe('models.list', () => {
    test('Paginates results', async () => {
      nock(BASE_URL)
        .get('/models')
        .reply(200, {
          results: [{ url: 'https://dashscope.com/some-user/model-1' }],
          next: 'https://api.dashscope.com/v1/models?cursor=cD0yMDIyLTAxLTIxKzIzJTNBMTglM0EyNC41MzAzNTclMkIwMCUzQTAw',
        })
        .get('/models?cursor=cD0yMDIyLTAxLTIxKzIzJTNBMTglM0EyNC41MzAzNTclMkIwMCUzQTAw')
        .reply(200, {
          results: [{ url: 'https://dashscope.com/some-user/model-2' }],
          next: null,
        });
      ...
      expect(results).toEqual([{ url: 'https://dashscope.com/some-user/model-1' }, { url: 'https://dashscope.com/some-user/model-2' }]);
  ...
  describe('predictions.create', () => {
    ...
    test('Does not automatically retry on 500', async () => {
      ...
      ).rejects.toThrow(`Request to https://api.dashscope.com/v1/predictions failed with status 500 Internal Server Error: {"detail":"Internal server error"}.`);
    });
  });
    ...
    test('Calls the correct API route with the correct payload', async () => {
      nock(BASE_URL)
        ...
        .reply(200, {
          ...
          urls: {
            get: 'https://api.dashscope.com/v1/predictions/rrr4z55ocneqzikepnug6xezpe',
            cancel: 'https://api.dashscope.com/v1/predictions/rrr4z55ocneqzikepnug6xezpe/cancel',
          },
          ...
          output: ['https://dashscope.com/api/models/stability-ai/stable-diffusion/files/9c3b6fe4-2d37-4571-a17a-83951b1cb120/out-0.png'],
          error: null,
  ...
  describe('deployments.predictions.create', () => {
    test('Calls the correct API route with the correct payload', async () => {
      nock(BASE_URL)
        .post('/deployments/dashscope/greeter/predictions')
        .reply(200, {
      ...
      const prediction = await client.deployments.predictions.create('dashscope', 'greeter', {
        input: {
          text: 'Alice',
        },
        webhook: 'http://test.host/webhook',
